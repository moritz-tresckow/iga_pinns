...
class PatchNURBS(Patch):
    ...
    def __call__(self, y, differential=False):
        ...
        Bs = [b(y[:,i]) for i,b in enumerate(self.basis)] # enumerate and evaluate the basis. Return list with Bs = [Bs_x(x), Bs_y(y)], length = 2
        
        print('The shape of the weights is: ', self.weights.shape)
        print('The shape of the initial Basis is ', Bs[0].shape)
        print('The shape of the second Basis is ', Bs[1].shape)
        den = np.einsum('im,i...->m...',Bs[0],self.weights)
        print('The shape of the Einstein sum is: ', den.shape)
        print('The shape of the knots is: ', self.knots.shape)
        print(self.knots)
    ...
    def quadrature(self, N = 32, knots = 'leg'):
        ...
        print('The length of the knot vector is ', len(Knots), ' and the number of elements is ', Knots[0].shape)
...
class PatchNURBSParam(Patch):
    ...
    __weights: Callable or Array
    __knots: Callable or Array
    ...
    def knots(self, params: Array or None = None) -> Array:
        if params is None:
            return self.__knots
        else: 
            return self.__knots(params)
    
    def weights(self, params: Array or None = None) -> Array:
        if params is None:
            return self.__weights
        else: 
            return self.__weights(params)
    
    def __init__(self, basis : list[BSplineBasisJAX], knots : Array or Callable, weights: Array or Callable, dparam : int, dembedding :int, rand_key: jax.random.PRNGKey, bounds = None):
    ...
    def __call__(self, y: Array, params: Array or None = None, differential: bool =False) -> Array or tuple[Array, Array]:
    ...
    def _eval_derivative(self, y: Array, params: Array or None, dim: int)->Array:
    ...
    def GetJacobian(self, y: Array, params: Array or None = None) -> jax.Array:
        """
    ...
    def GetMetricTensors(self, y: Array, params: Array or None = None) -> tuple[jax.Array,jax.Array,jax.Array]:
    ...
    def _eval_omega(self,y: Array, params:Array or None = None):
        """
        Evaluate the derivative of the parametrization (jacobian).
        y[...,i,j] = \partial_y
