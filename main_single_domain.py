...
def mke_domain(rnd_key):  
    geoms, knots = mke_domains(rnd_key)
    meshfile = './fem_ref/fenicsx_mesh/quad_dom12/quad_dom12'
    return geoms, knots, meshfile

def mke_domain_simple(rnd_key):  
    geoms, knots = create_geometry(rnd_key)
    meshfile = './fem_ref/fenicsx_mesh/quad_dom12/quad_simple_dom12' 
    return geoms, knots, meshfile

def mke_domain_simple_alt(rnd_key):  
    geoms, knots = create_geometry_alt(rnd_key)
    meshfile = './fem_ref/fenicsx_mesh/quad_dom12/quad_simple_dom12_alt' 
    return geoms, knots, meshfile

geoms, knots, meshfile = mke_domain_simple_alt(rnd_key)
...
stepsize = 0.00001                                                           # Stepsize for Optimizer aka. learning rate
...
evaluate_error(model, params, evaluate_single_model, [0,1], geoms, meshfile)
