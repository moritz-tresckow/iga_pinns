...
def create_geo_II(key):
    ...
    print(knots_upper, knots_lower)
    knots_middle = 0.5*(knots_upper + knots_lower)

    knots = np.concatenate((knots_lower[None,...],knots_middle[None,...],knots_upper[None,...]),0)
    weights = np.ones(knots.shape[:2])
    
    basisx = src.bspline.BSplineBasisJAX(np.linspace(-1,1,3),1)
    basisy = src.bspline.BSplineBasisJAX(np.array([-1,-0.33,0.33,1]),1)
    geom4 = src.geometry.PatchNURBSParam([basisx, basisy], knots, weights, 0, 2, key)





    knots_upper5 =  np.array([[startx, starty], [startx + 0.75*dd, starty + 0.75*dd], [startx + (0.75 + 1.3876)*dd, starty + 0.75*dd], [startx + (1.5 + 1.3876)*dd, starty]])
    knots_lower5 = np.array([[startx, 0], [startx + 0.75*dd, 0], [startx + (0.75 + 1.3876)*dd, 0], [startx + (1.5 + 1.3876)*dd, 0]])





    knots_lower5 = np.flip(knots_lower5, axis = 0)
    knots_upper5 = np.flip(knots_upper5, axis = 0)
    knots5 = np.concatenate((knots_lower5[None,...], knots_upper5[None,...]),0)

    weights5 = np.ones(knots5.shape[:2])
    
    basisx = src.bspline.BSplineBasisJAX(np.linspace(-1,1,2),1)
    basisy = src.bspline.BSplineBasisJAX(np.array([-1,-0.33,0.33,1]),1)
    geom5 = src.geometry.PatchNURBSParam([basisx, basisy], knots5, weights5, 0, 2, key)


    return [geom4, geom5], [knots, knots5]
def mke_geo(rnd_key):  
    geoms, knots = create_geometry_alt(rnd_key)
    meshfile = './fem_ref/fenicsx_mesh/quad_dom1234/quad_dom1234' 
...
class Model(src.PINN):
    def __init__(self, rand_key):
        ...
        self.add_flax_network('u25', feat_bndr, act_bndr, load, path)
        ...
        self.interface25 = interface_function2d(1,-1.0,1.0,self.neural_networks['u25'])
        self.interface52 = interface_function2d(1,1.0,-1.0,self.neural_networks['u25'])
        ...
        self.add_trainable_parameter('u2345',(1,), load_p, path) 
    ...
    def solution2(self, ws, x):
        ...
        w25 = self.interface25(ws['u25'], x) * (1 - x[...,0])[...,None]         

        w123   = ws['u123']  *( (x[...,0] + 1) * (x[...,1] + 1) )[...,None]**alpha
        w2345  = ws['u2345'] *( (x[...,0] + 1) * (1 - x[...,1]) )[...,None]**alpha
    ...
    def solution3(self, ws, x):
        ...
        w123   = ws['u123']  *( (x[...,0] + 1) * (x[...,1] + 1) )[...,None]**alpha
        w2345  = ws['u2345'] *( (1 - x[...,0]) * (x[...,1] + 1) )[...,None]**alpha
    ...
    def solution4(self, ws, x):
        ...
        w2345  = ws['u2345'] *( (x[...,0] + 1) * (x[...,1] + 1) )[...,None]**alpha
    ...
    def solution5(self, ws, x):
        ...
        w52 = self.interface52(ws['u25'], x) * ((1 - x[...,0]))[...,None]
    ...
    def loss_pde(self, ws, points):
        ...
        return lpde1 + lpde2 + lpde3 + lpde4 + lpde5
...
batch_size = 40000                                                          # Number of sample points for quadrature (MC integration) 
...
evaluate_error(model, params, evaluate_models5, [0,1,2,3,4], geoms, meshfile)
exit()
