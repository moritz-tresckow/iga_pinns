...
def create_geo_II(key):
    ...
    knots_upper = np.array([l1, l2, l3 ,l4])

    l1 = [startx + 0.5*dd               , 0 ]
    l2 = [startx + (0.75 + 1.3876/2)*dd , 0 ]
    l3 = [startx + (0.75 + 1.3876)*dd   , 0 ]
    l4 = [startx + (1.5 + 1.3876)*dd    , 0 ]
    knots_lower = np.array([l1, l2, l3 ,l4])
    
    knots6 = np.concatenate((knots_lower[None,...], knots_upper[None,...]),0)
...
class Model(src.PINN):
    ...
    def solution6(self, ws, x):
        ...
        w65 = self.interface65(ws['u56'], x) *  (x[...,1] + 1)[...,None]
        w67 = self.interface67(ws['u67'], x) *  (1 - x[...,0]) [...,None]
        
        w567  = ws['u567'] *( (x[...,0] + 1) * (1 - x[...,1]) )[...,None]**alpha
