...
b1 = src.bspline.BSplineBasis(np.linspace(0,1,4),2)
b2 = src.bspline.BSplineBasis(np.linspace(0,1,2),1)
...
geom = src.geometry.PatchNURBS([b2, b1], knots, weights, key)
...
geom = src.geometry.PatchNURBS([src.bspline.BSplineBasis(np.linspace(0,1,2),2), src.bspline.BSplineBasis(np.linspace(0,1,2),2)],knots,weights,key)
...
class Model(src.PINN):
    ...
    def loss_pde(self, ws):
        grad = src.operators.gradient(lambda x : self.solution(ws,x))(self.points['ys'])
        nu = 1/3
...
result = scipy.optimize.minimize(loss_grad, x0 = np.array(weights), method = 'L-BFGS-B', jac = True, tol = 1e-9, options = {'disp' : True, 'maxiter' : 10000, 'iprint': 1})
